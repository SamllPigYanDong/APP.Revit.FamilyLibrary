...
{
    public class FamilyLibraryManagerViewModel : ViewModelBase
    {
        ...
        private async void DeleteCategory(ViewCategoryDto category)
        {
            ...
            {
                var result = await categoryWebService.DeleteCategory(category.Id);
                if (result>0)
                {
                    var parent=categoryService.GetCategories(Categories.FirstOrDefault()).FindFirst(x => x.Id == category.ParentId);
                    if (parent!=null)
                    {
        ...
        private void AddCategory(ViewCategoryDto parent)
        {
            ...
            this.dialogService.ShowDialog(nameof(CreateCatagoryDialogView), parameters, async (dialogResult) =>
            {
                if (dialogResult.Result == ButtonResult.Yes)
                {
                    ...
                    var result = await categoryWebService.AddCategory(createCategory);
                    if (result!=null )
                    {
                        if (parent != null)
                        {
        ...
        public FamilyLibraryManagerViewModel( ICategoryService categoryService, IDialogService dialogService,ICategoryAppService categoryWebService)
        {
